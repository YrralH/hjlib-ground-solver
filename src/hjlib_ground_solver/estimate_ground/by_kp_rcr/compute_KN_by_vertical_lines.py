...
def assert_zeros(array: np.ndarray, thres: float = 1e-6) -> None:
    ...
    assert np.max(array) < thres, np.max(array)
...
def get_bias_from_2D_ground_normal(xt: np.ndarray, xb: np.ndarray, KN: np.ndarray) -> List[float]:
    ...
    bias_cos = np.zeros((NUM_VECTOR,), dtype=np.float32)
...
def get_KN_with_filter(
    ...
    def loop_filter(_xb: np.ndarray, _xt: np.ndarray, _prop_keep: float = 0.2) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        _max_bias = _list_bias_to_KN_sorted[int(_num * _prop_keep)]

        # the function is the same as a per-value filter, but the implementation is more general
        _list_valid_mask = get_valid_filter_mask_by_max_value(_list_bias_to_KN_sorted, _max_bias)
    ...
    for ____ in range(times_filter):
        xb_tmp, xt_tmp, ____ = loop_filter(xb_tmp, xt_tmp, _prop_keep=1 - prop_filter)
    KN_RET = get_KN(xb_tmp, xt_tmp)
